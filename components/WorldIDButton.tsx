...
export default function WorldIDButton({ appId, onSuccess }: Props) {
  ...
  if (status === 'waiting' && qrUrl) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <p style={{ fontFamily: 'IBM Plex Mono', fontSize: 11, color: '#666', textAlign: 'center', margin: 0 }}>
        Scan with World App on your phone
      </p>
      <img src={qrUrl} alt="World ID QR" style={{ width: 180, height: 180, background: '#fff', padding: 8 }} />
      <p style={{ fontFamily: 'IBM Plex Mono', fontSize: 10, color: '#444', margin: 0 }}>
        Waiting for verification...
      </p>
      <button
        onClick={() => { setStatus('idle'); setQrUrl(null) }}
        style={{ background: 'none', border: 'none', color: '#444', fontFamily: 'IBM Plex Mono', fontSize: 10, cursor: 'pointer' }}
      >
        Cancel
      </button>
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {status === 'error' && (
        <p style={{ color: '#ff4444', fontFamily: 'IBM Plex Mono', fontSize: 11, textAlign: 'center', margin: 0 }}>
          {errorMsg}
        </p>
      )}
      <button
