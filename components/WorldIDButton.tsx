...
export default function WorldIDButton({ appId, onSuccess }: Props) {
  ...
  const startVerification = async () => {
    ...
    try {
      ...
      const poll = async (): Promise<void> => {
        if (attempts >= 150) { setStatus('idle'); setQrUrl(null); setDeepLink(null); return }
        attempts++
        try {
          ...
          if (result) {
            ...
            if (data.verified) { setStatus('done'); onSuccess(data) }
            else { setStatus('error'); setErrorMsg(data.detail || 'Verification failed. Try again.') }
          }
        } catch (e: any) {
          const msg = e?.message || ''
          if (msg.includes('expired') || msg.includes('timeout') || msg.includes('cancel')) {
            setStatus('idle'); setQrUrl(null); setDeepLink(null)
          } else if (msg.includes('pending') || msg.includes('waiting')) {
            setTimeout(poll, 2000)
          } else {
            setStatus('error'); setErrorMsg('Scan failed. Please try again.'); setQrUrl(null); setDeepLink(null)
          }
        }
      }
      setTimeout(poll, 2000)
  ...
  if (status === 'done') return (
    <div style={{ textAlign: 'center', color: '#00ff87', fontFamily: 'IBM Plex Mono', fontSize: 13 }}>checkmark Verified</div>
  )

  if (status === 'waiting' && qrUrl && deepLink) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      {isMobile ? (
        <>
          ...
          <a href={deepLink} style={{ width: '100%', display: 'block', textAlign: 'center', padding: '14px', background: 'rgba(0,255,135,0.1)', border: '1px solid #00ff87', color: '#00ff87', fontFamily: 'IBM Plex Mono', fontSize: 13, textDecoration: 'none', fontWeight: 600 }}>
            Open World App to Verify
          </a>
          <p style={{ fontFamily: 'IBM Plex Mono', fontSize: 9, color: '#333', margin: 0, textAlign: 'center' }}>or scan below with another device</p>
          <img src={qrUrl} alt="World ID QR" style={{ width: 160, height: 160, background: '#fff', padding: 8, opacity: 0.5 }} />
        </>
      ) : (
        <>
          <a href={deepLink} style={{ fontFamily: 'IBM Plex Mono', fontSize: 12, color: '#ffffff', textAlign: 'center', margin: 0, textDecoration: 'underline', cursor: 'pointer' }}>
      ...
      <button onClick={() => { setStatus('idle'); setQrUrl(null); setDeepLink(null) }} style={{ background: 'none', border: 'none', color: '#444', fontFamily: 'IBM Plex Mono', fontSize: 10, cursor: 'pointer' }}>Cancel</button>
    </div>
  )
